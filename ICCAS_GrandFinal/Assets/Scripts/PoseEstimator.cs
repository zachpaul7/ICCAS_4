...
public class PoseEstimator : MonoBehaviour
{
    ...
    [HideInInspector] public Utils.Keypoint[] keypoints;
    ...
    void Start()
    {
        ...
        if (useWebcam)
        {
            ...
            // Iterate through available devices to find the front-facing camera
            for (int cameraIndex = 0; cameraIndex < WebCamTexture.devices.Length; cameraIndex++)
            {
                if (WebCamTexture.devices[cameraIndex].isFrontFacing)
                {
                    webcamTexture.deviceName = WebCamTexture.devices[cameraIndex].name;
                    break;
                }
            }
    ...
    private void ProcessOutput(IWorker engine)
    {
        ...
        if (estimationType == EstimationType.SinglePose)
        {
            ...
            this.gameObject.GetComponent<PoseEvaluation>().Init();
        }
        else
        {
            // Determine the key point locations
